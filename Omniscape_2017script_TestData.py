...
# for now ignore climate... would need to mask those too
...
options['projectDir'] = r'N:\Projects\Connectivity\projects\OmniscapeTestData' #r'C:\DATADRIVE\DUKE_PNW_DATA\PNW_OmniScape'# this is where all the input data are, and where out directory will be created.
options['resisRasterBase'] = 'test_resistance_x10k_USGS.tif'#'ones_oly.tif'#'rNullFadeTestClip2.tif'#'ones.tif'#
options['outputDirBase'] = 'test_output2' #'nullR_nullS_810_50km'
options['runTitle'] = options['outputDirBase']
...
options['sourceRasterBase'] = 'test_source_USGS.tif' #'null_810.tif'#'DUKE_CIRCUITSCAPE_resistances_draft4_Habitat_min_540m_clip.tif'#'srcInvR.tif'# Name of source raster, if using one
...
options['maskRasterBase'] = 'ca_50km_270_1.tif'
...
options['usePCDistFn'] = True # Still have a PC window (cutoff), but flow is scaled so that there is more flow with closer matches. See equation that is invoked when this option is set to True.
# parameters to match temperature differences
options['tDiff'] = 4 # Match pixels that differ in temperature by this value
options['tWindow'] = 2 #How close does target need to be to the tDiff value
options['climateRasterBase'] = 'tmean_adks.asc'#'5x5t1pc2.asc'#'5x5clim.asc'#'TMEAN_NE_clip.tif'
...
options['t1PC1RasterBase'] = 'ca_pca1_current_812010_1.tif'
options['t1PC2RasterBase'] = 'ca_pca2_current_812010_1.tif'
options['t2PC1RasterBase'] = 'ca_pca1_HadGEM2-ES_rcp85_2055_1.tif'
options['t2PC2RasterBase'] = 'ca_pca2_HadGEM2-ES_rcp85_2055_1.tif'
options['PCWindow'] = 1.5 # Euclidean distance between PCs to determine match
...
options['startBand'] = 0#127#4#16#4# First band to process. Use 0 to ignore.
options['endBand'] = 0#199#15# # Stops after processing this band. Use 0 to ignore.
options['startStripe'] = 0#76 # First stripe to process. Use 0 to ignore.
options['endStripe'] = 0#156
...
options['printTimings'] = False

options['cleanUpBandFiles'] = True

options['useCustomCSPath'] = False # If Circuitscape isn't installed in the usual place, e.g. C:\Program Files\Circuitscape\cs_run.exe, set this to True and use next line to specify a custom location.
options['CSPath'] = r'C:\Program Files\Circuitscape\cs_run.exe' # Set custom CS path here if line above is True
...
# Have gdal importing in win32 Arc Python installation. Holding off migration for now.
...
def omniscape(options):
    try:
        ...
        if options['useMask']:
            options = create_masked_inputs(options)
...
#GDAL TEST
        # ds1 = gdal.Open(resisRaster , GA_ReadOnly )
        # bandNum1 = 1
        # band1 = ds1.GetRasterBand(bandNum1)
        # data1 = BandReadAsArray(band1) # converts to Numpy array
        # print 'SIZE'
        # print data1.shape
...
#fixme: should add 1 to the upper index in next line?
    ...
    #fixme- check this:
                ...
                # print sourceArray
                ### CARRIE'S NOTE - WHAT DOES BLARG DO? I COMMENTED THIS OUT BECAUSE IT WAS THROWING ERRORS - OK TO DO?
				# blarg
    ...
    #put in fn once striping solved below
                ...
                if options['calcVoltages']:
                    ...
                    # power = get_power(voltMap,circleResisArray,csOptions)
    ...
    # FIXME: move file writing to band iteration
        ...
        # Fixme: put below in finally statement
        if options['useMask']: 
            delete_data(resisRaster)
            if options['useSourceRaster']:
                delete_data(sourceRaster)
        return options
...
def clear_prof_data():
    ...
    if options['useClimate']:
        print 'Error: climate code is not yet compatible with mask code. Please set useMask to False.'
        raw_input('\nPress Enter to continue.') 
        exit(0)          
        
def create_masked_inputs(options):
    try:
        resisRaster = path.join(options['projectDir'],options['resisRasterBase'])

        maskRaster = path.join(options['projectDir'], options['maskRasterBase'])
        # maskRaster = path.join(options['scratchDir'], options['maskRasterBase'])
        
        # arcpy.Resample_management (maskRaster1, maskRaster, {cell_size}, {resampling_type})
        descData = arcpy.Describe(maskRaster)
        arcpy.env.extent = descData.Extent    
        
        descData = arcpy.Describe(resisRaster)
        spatialReference = descData.spatialReference
        arcpy.env.cellSize = descData.MeanCellHeight
        arcpy.env.snapRaster = resisRaster
        
        resisRasterMasked = arcpy.sa.Con(arcpy.sa.Raster(maskRaster)>0,resisRaster) 
        
        fileBase, fileExtension = path.splitext(options['resisRasterBase'])
        newResisRasterBase = 'mask_'+fileBase + str(options['tileNum']) + '.tif'
        resisRasterMaskFile = path.join(options['projectDir'],newResisRasterBase)
        resisRasterMasked.save(resisRasterMaskFile)           
        options['resisRasterBase'] = newResisRasterBase
        
        if options['useSourceRaster']:
            sourceRaster = path.join(options['projectDir'],options['sourceRasterBase'])
            fileBase, fileExtension = path.splitext(options['sourceRasterBase'])
            sourceRasterMasked = arcpy.sa.Con(arcpy.sa.Raster(maskRaster)>0,sourceRaster) 
            newSourceRasterBase = 'mask_'+fileBase + str(options['tileNum']) + '.tif'
            sourceRasterMaskFile = path.join(options['projectDir'],newSourceRasterBase)
            sourceRasterMasked.save(sourceRasterMaskFile)           
            options['sourceRasterBase'] = newSourceRasterBase
        return options
    ...
    try:
        ...
        for i in range(0,len(tRows)):
            ...
            if options['usePCDistFn']:
                ...
                PCCutoffArray = npy.where((PCDistArray<= options['PCWindow']),(options['PCWindow'] - abs(PCDistArray))/options['PCWindow'],0) 
                #fixme: clunky
                PCCutoffArray = npy.where((PCDistArray < 0),0,PCCutoffArray) 
            else:
                PCCutoffArray = npy.where((PCDistArray<= options['PCWindow']),1,0) 
                PCCutoffArray = npy.where((PCDistArray < 0),0,PCCutoffArray) 
            if len(sourceArray) < 10:
                print 'PCCutoffArray'
                print PCCutoffArray
...
def get_power(voltMap,circleResisArray,csOptions):
    try:
        print "WARNING: only calculating 4 directions so far"
        ...
        if len(voltMap) < 10:
            ...
            print 'resis_u'
            print resis_u        
        partialV_up = get_partial_v(voltMap, voltMap_u, resis_u, circleResisArray)
       
        # print 'voltMap_l'
        # print voltMap_l
        
# For each cell, want sum of voltages
# want sum of currents (got it)
# want sum of powers (need 8 INDIVIDUAL voltages and cell resistance) v2/r
...
def get_cs_path():
    ...
    try:
        if options['useCustomCSPath']:
            csPath = options['CSPath'] 
        ...
        csPath = 'E:\\Program Files\\Circuitscape\\cs_run.exe'        
        if os.path.exists(csPath): return csPath
        print 'Error: unable to find Circuitscape installation in any of the usual places (such as C:\Program Files\Circuitscape\cs_run.exe).'
        print 'You can define a custom Circuitscape installation location in the user settings.'
        raw_input('\nPress Enter to continue.') 
        exit(0)    
